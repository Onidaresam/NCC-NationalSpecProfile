...
import BandsData from '../../Data/Technology';
...
export default function Body() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!payload.location || (!payload.region && state === 'Lagos') || !payload.bands) {
      setLoading(false);
      toast({
        title: 'Error',
        variant:'top-accent',
    ...
    function getResultObject(data) {
      ...
      let technology;
      if (region) {
        technology = locationData.region.find((r) => r.region === region)?.technologies.find((b) => b.name === bands);
      } else {
        technology = locationData.technologies.find((b) => b.name === bands);
      }
      if (!technology) {
      ...
      const subband = technology.bands.find((sb) => sb.range === subbandss);
      if (!subband) {
  ...
  return (
    ...
    <div className=' flex flex-col lg:hidden'>
      ...
      <form onSubmit={handleSubmit} className='relative mx-[16px] bottom-[100px] left-0 right-0
          ...
          <div className='flex flex-col space-y-[24px]'>
            ...
            <div>
              ...
              <div className='p-[16px] w-full rounded-[10px] border border-solid border-[#CECEDE] bg-white focus:border-[#73FF0F]'>
                <Select name="location" placeholder='Select State' variant='unstyled' size="lg" className='w-full' onChange={handleStateChange}>
             ...
             {state ==="Lagos" && <div>
              <h1>Region</h1>
              <div className='p-[16px] w-full rounded-[10px] border border-solid border-[#CECEDE] bg-white focus:border-[#73FF0F]'>
                <Select name="region" placeholder='Select Region' variant='unstyled' size="lg" className='w-full'  onChange={handleRegionChange}>
                    {BandsData.find((data) => data.location === state)?.region?.map((region, index) => (
            ...
            <div>
              ...
              <div className='p-[16px] w-full rounded-[10px] border border-solid border-[#CECEDE] bg-white focus:border-[#73FF0F]'>
              {state === "Lagos" ? (
                  <Select name="bands" placeholder='Select Band' variant='unstyled' size="lg" isDisabled={checkStateAndRegion(state, region)}
                  className='w-full' onChange={handleBandChange}>
                   {BandsData.find((data) => data.location === state)?.region.find((r)=>r.region === region)?.technologies?.map((band) => (
                  ...
                  <Select name="bands" placeholder='Select Band' variant='unstyled' size="lg" isDisabled={checkStateAndRegion(state, region)}
                    className='w-full' onChange={handleBandChange}>
                     {BandsData.find((data) => data.location === state)?.technologies?.map((band) => (
                       <option key={band.name} value={band.name}>
                         {band.name}
                       </option>
                     ))}
            ...
            <div>
              {
                state !="Lagos" && <div className='p-[16px] w-full rounded-[10px] border border-solid border-[#CECEDE] bg-white focus:border-[#73FF0F] '>
                  ...
                  {BandsData.find((data) => data.location === state)?.technologies?.find((bands) => bands.name === band)?.bands?.map((subband) => 
                    (
                ...
                className='w-full'  
                onChange={handleSubbandChange}>
                {BandsData.find((data) => data.location === state)?.region.find((r)=>r.region === region)?.technologies?.find((bands) => bands.name === band)?.bands?.map((subband) => 
                  (
                    <option key={subband.range} value={subband.range}>
                      {subband.range}
                    </option>
      ...
      {result ? (
        <div ref={spectrumRef} className='mt-[100px] mx-[16px] md:mx-[45px] flex flex-col'>
          ...
          <div className='border-2 rounded-[10px] py-[40px] px-[16px]'>
            ...
            <div className='flex flex-col space-y-[16px]'>
              <h1 className='font-graphik font-semibold text-[20px] leading-[28px]'>Utilization</h1>
              <div className='flex flex-col p-[16px] space-y-[24px]'>
                <h1 className='font-graphik font-normal text-[20px] leading-[28px]'>Duty cycle (percentage)</h1>
                <p className='rounded-[8px] bg-[#F6F6F8] py-[12px] px-[24px] font-graphik font-normal text-[32px] leading-[40px] mr-[150px]'>{result?.utilization}%</p>
              </div>
            </div>
            {/*Frequency */}
            <div className='flex flex-col space-y-[16px]'>
              ...
              <div className='flex flex-col space-y-[40px]'>
                {/*Utilized */}
                <div>
                  <h1 className='font-graphik font-semibold leading-[24px] text-[16px] 
                    bg-[#F6F6F8] border rounded-[10px] py-[16px] px-[16px]'>Utilized</h1>
                  <div className='border rounded-[10px] py-[24px] px-[16px] flex flex-col space-y-[24px]'>
                    {getUtilizedFrequenciesByPage()?.map((frequency, index) => (
                      <p key={index} className='font-graphik text-[24px] font-normal leading-[32px]'>
                        {frequency}
                      </p>
                    ))}
                    </div>
                ...
                <div>
                  <h1 className='font-graphik font-semibold leading-[24px] text-[16px] 
                    bg-[#F6F6F8] border rounded-[10px] py-[16px] px-[16px]'> Not Utilized</h1>
                  <div className='border rounded-[10px] py-[24px] px-[16px] flex flex-col space-y-[24px]'>
                    {getNotUtilizedFrequenciesByPage()?.map((frequency, index) => (
                      <p key={index} className='font-graphik text-[24px] font-normal leading-[32px]'>
                        {frequency}
                      </p>
                    ))}
        ...
        <div ref={spectrumRef} className='mt-[100px] mx-[16px] md:mx-[45px] flex flex-col'>
          ...
          <div className='border rounded-[10px] p-[40px]  h-[500px] flex flex-col items-center justify-center'>
            <img src={none}/>
            <p className='font-graphik font-medium text-[16px] leading-[24px]'>No available data</p>
          </div>
        </div>
    ...
    <div className='hidden lg:flex flex-col'>
      ...
      <form onSubmit={handleSubmit} className='relative lg:bottom-[250px] xl:bottom-[140px] left-0 right-0 mx-[172.5px] p-[40px] rounded-[20px] border-2 border-[#EDEDF2] bg-white shadow-spec'>
        <div className='flex flex-col space-y-[24px] '>
          {/*Location */}
          <div className='flex flex-col'>
            <h1 className='text-[#060A23] font-graphik text-base font-medium leading-6'>Location</h1>
          ...
          {/*Bands */}
          <div  className='flex flex-col'>
            <h1 className='text-[#060A23] font-graphik text-base font-medium leading-6'>Technologies</h1>
            <div className='h-[86px] items-center justify-center flex flex-row space-x-[24px] w-full'>
              <div className='p-[16px] w-full rounded-[10px] border border-solid border-[#CECEDE] bg-white focus:border-[#73FF0F] '>
                {state === "Lagos" ? (
                  <Select name="bands" placeholder='Select Band' variant='unstyled' size="lg" isDisabled={checkStateAndRegion(state, region)}
      ...
      {result ? (
        ...
        <div className='border-2 rounded-[10px] p-[40px]'>
          <div className='flex flex-col space-y-[40px]'>
            {/*DUTY CYCLE*/}
            <div className='flex flex-col'>
              <h1 className='font-graphik text-[24px] font-bold leading-[32px]'>Utilization</h1>
              <div className='p-[24px] flex flex-row space-x-[100px] items-center'>
                <p className='font-graphik text-[36px] font-normal leading-[44px]'>Duty cycle (percentage) :</p>
                <p className='rounded-[8px] bg-[#F6F6F8] py-[12px] px-[24px] font-graphik text-[48px] leading-[56px] font-normal'>{result?.utilization}%</p>
              </div>

            </div>
            {/*Frequency */}
            <div className='flex flex-col space-y-[16px]'>
              <h1 className='font-graphik text-[24px] font-bold leading-[32px]'>Frequencies</h1>
                <div className='flex flex-row space-x-[40px] w-full'>
                  {/*Utilized */}
                  <div className='flex flex-col w-[50%]'>
                    <p className='font-graphik font-bold leading-[28px] text-[20px] bg-[#F6F6F8] border rounded-[10px] py-[16px] px-[24px]'>Utilized</p>
                    ...
                    <ReactPaginate className='flex flex-row p-[15px] mt-[10px] justify-center space-x-[24px]'
                      previousLabel={'<<<'}
                      nextLabel={'>>>'}
                      breakLabel={'...'}
                      pageCount={Math.ceil(result?.frequencies.utilized.length / frequenciesPerPage)}
                  ...
                  {/*UNUTILIZED */}
                  <div className='flex flex-col w-[50%]'>
                  <p className='font-graphik font-bold leading-[28px] text-[20px] bg-[#F6F6F8] border rounded-[10px] py-[16px] px-[24px]'>Not Utilized</p>
                    <div className='border rounded-[10px] p-[24px] flex flex-col space-y-[24px]'>
                    {getNotUtilizedFrequenciesByPage().map((frequency, index) => (
                      <p key={index} className='font-graphik text-[24px] font-normal leading-[32px]'>
                        {frequency}
                    ...
                    <ReactPaginate className='flex flex-row p-[15px] mt-[10px] justify-center space-x-[24px]'
                      ...
                      nextLabel={'>>>'}
                      breakLabel={'...'}
                      pageCount={Math.ceil(result?.frequencies.notutilized.length / frequenciesPerPage)}
                      marginPagesDisplayed={2}
                      pageRangeDisplayed={5}
