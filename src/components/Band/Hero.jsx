...
export default function Hero() {
  ...
  const handleSubmit = async (event) => {
    ...
    function getResultObject(data) {
      ...
      let band;
      if (region) {
        band = locationData.region.find((r) => r.region === region)?.bands.find((b) => b.range === bands);
      } else {
    ...
    <div className=' flex flex-col lg:hidden'>
      ...
      md:mx-[45px] py-[24px] px-[16px] rounded-[20px] border-2 border-[#EDEDF2] bg-white shadow-spec'>
          <div className='flex flex-col space-y-[24px]'>
            ...
            <div>
              ...
              {state === "Lagos" ? (
                  ...
                  className='w-full' onChange={handleBandChange}>
                   {BandsData.find((data) => data.location === state)?.region.find((r)=>r.region === region)?.bands?.map((band) => (
                  ...
                  <Select name="bands" placeholder='Select Band' variant='unstyled' size="lg" isDisabled={checkStateAndRegion(state, region)}
                    className='w-full' onChange={handleBandChange}>
                     {BandsData.find((data) => data.location === state)?.bands?.map((band) => (
                       <option key={band.range} value={band.range}>
                         {band.range}
                       </option>
                     ))}
            ...
            <div>
              {
                band !=="0-30Mhz" && state !="Lagos" && <div className='p-[16px] w-full rounded-[10px] border border-solid border-[#CECEDE] bg-white focus:border-[#73FF0F] '>
                  ...
                  {BandsData.find((data) => data.location === state)?.bands?.find((bands) => bands.range === band)?.subbands?.map((subband) => 
                    (
                ...
                className='w-full'  
                onChange={handleSubbandChange}>
                {BandsData.find((data) => data.location === state)?.region.find((r)=>r.region === region)?.bands?.find((bands) => bands.range === band)?.subbands?.map((subband) => 
                  (
                    <option key={subband.range} value={subband.range}>
                      {subband.range}
                    </option>
    ...
    <div className='hidden lg:flex flex-col'>
      ...
      <form onSubmit={handleSubmit} className='relative lg:bottom-[250px] xl:bottom-[140px] left-0 right-0 mx-[172.5px] p-[40px] rounded-[20px] border-2 border-[#EDEDF2] bg-white shadow-spec'>
        <div className='flex flex-col space-y-[24px] '>
          ...
          {/*Bands */}
          <div  className='flex flex-col'>
            <h1 className='text-[#060A23] font-graphik text-base font-medium leading-6'>Bands</h1>
            <div className='h-[86px] items-center justify-center flex flex-row space-x-[24px] w-full'>
              <div className='p-[16px] w-full rounded-[10px] border border-solid border-[#CECEDE] bg-white focus:border-[#73FF0F] '>
                {state === "Lagos" ? (
                  <Select name="bands" placeholder='Select Band' variant='unstyled' size="lg" isDisabled={checkStateAndRegion(state, region)}
